...
async function executeMigration() {
  ...
  try {
    ...
    const migrationFile = path.join(__dirname, 'backend/migrations/030_add_zpl_templates_to_health_check_settings.sql');
    ...
    console.log('✅ Migration 030 executed successfully!');
    ...
    const resColumns = await client.query(`
      ...
      WHERE table_name = 'health_check_settings' 
        AND column_name IN ('barcode_zpl_template_xn', 'barcode_zpl_template_ksk', 'barcode_printer_name')
    `);
    
    console.log('\n--- VERIFICATION ---');
    resColumns.rows.forEach(col => {
      console.log(`Column ${col.column_name} exists with type ${col.data_type}`);
    });
