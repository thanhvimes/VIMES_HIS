...
const PdfPreviewModal: React.FC<PdfPreviewModalProps> = ({ isOpen, onClose, pdfUrl, fileName, isSignable = false, onSign, signatures = [], onDeleteSignature }) => {
  ...
  return (
    <>
      <div 
        className={`fixed inset-0 bg-black z-[2000] flex justify-center items-center p-2 sm:p-4 transition-opacity duration-300 ease-out ${isShowing ? 'bg-opacity-80' : 'bg-opacity-0'}`} 
        aria-modal="true" 
        role="dialog"
      >
        <div 
      ...
      {viewingSignature && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-[2200] flex justify-center items-center p-4" onClick={() => setViewingSignature(null)}>
