...
async function executeMigration() {
  ...
  try {
    ...
    const migrationFile = path.join(__dirname, 'backend/migrations/017_pacs_native_tables.sql');
    ...
    // Verify tables
    const resTemplates = await client.query("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'hms_pacs_template_custom')");
    const resFavorites = await client.query("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'hms_pacs_favorites')");
    
    console.log('\n--- VERIFICATION ---');
    console.log('hms_pacs_template_custom exists:', resTemplates.rows[0].exists);
    console.log('hms_pacs_favorites exists:', resFavorites.rows[0].exists);
