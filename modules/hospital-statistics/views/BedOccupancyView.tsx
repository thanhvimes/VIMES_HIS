...
// Standardized to GCV Ninh Binh Medical UI System & Quyết Định 49/QĐ-BVĐKT (562 Giường KH)
...
import { TableEmptyState, PrintReportHeader, PrintReportFooter, exportTableToExcel, formatLocalDate } from '../components/CommonFilter';
import { statisticsService } from '../services/statisticsService';
import { BedOccupancyItem } from '../types';
import { BuildingOfficeIcon, UserGroupIcon, HeartIcon, SparklesIcon, ArrowPathIcon } from '../../../components/Icons';

type SortField = 'dept_name' | 'giuong_ke_hoach' | 'bn_noi_tru_bhyt' | 'bn_noi_tru_vienphi' | 'bn_ngoai_tru' | 'bn_dang_nam' | 'ty_le_cong_suat';
...
export const BedOccupancyView: React.FC = () => {
    const now = new Date();
    const [fromDate] = useState(`${formatLocalDate(now)} 00:00:00`);
    const [toDate] = useState(`${formatLocalDate(now)} 23:59:59`);
    ...
    // Aggregate totals (triệt tiêu số âm bằng Math.max(0, ...))
    const totals = useMemo(() => {
        return filteredItems.reduce((acc, curr) => ({
            giuong_ke_hoach: acc.giuong_ke_hoach + Math.max(0, Number(curr.giuong_ke_hoach || 0)),
            bn_noi_tru_bhyt: acc.bn_noi_tru_bhyt + Math.max(0, Number(curr.bn_noi_tru_bhyt || 0)),
            bn_noi_tru_vienphi: acc.bn_noi_tru_vienphi + Math.max(0, Number(curr.bn_noi_tru_vienphi || 0)),
            bn_ngoai_tru: acc.bn_ngoai_tru + Math.max(0, Number(curr.bn_ngoai_tru || 0)),
            bn_dang_nam: acc.bn_dang_nam + Math.max(0, Number(curr.bn_dang_nam || 0))
        }), { giuong_ke_hoach: 0, bn_noi_tru_bhyt: 0, bn_noi_tru_vienphi: 0, bn_ngoai_tru: 0, bn_dang_nam: 0 });
    }, [filteredItems]);
    ...
    const topOccupancyDepts = useMemo(() => {
        return [...items]
            .filter(it => Number(it.giuong_ke_hoach || 0) > 0)
        ...
        return [...filteredItems]
            ...
            .map(it => {
                ...
                return {
                    ...
                    rate: rate,
                    giuong_ke_hoach: kh,
                    bn_noi_tru_bhyt: Number(it.bn_noi_tru_bhyt || 0),
                    bn_noi_tru_vienphi: Number(it.bn_noi_tru_vienphi || 0),
                    bn_ngoai_tru: Number(it.bn_ngoai_tru || 0),
                    bn_dang_nam: bn,
                    diff: kh - bn
                };
            });
    ...
    const handleExport = () => {
        const rows = filteredItems.map((it, idx) => {
            const kh = Number(it.giuong_ke_hoach || 0);
            const bnBhyt = Math.max(0, Number(it.bn_noi_tru_bhyt || 0));
            const bnVp = Math.max(0, Number(it.bn_noi_tru_vienphi || 0));
            const bnNgoaiTru = Math.max(0, Number(it.bn_ngoai_tru || 0));
            const bn = Math.max(0, Number(it.bn_dang_nam || 0));
            return {
                'STT': idx + 1,
                'Mã Khoa': it.dept_id,
                ...
                'Tổng Số BN Đang Nằm': bn,
                'Công Suất Sử Dụng (%)': `${Number(it.ty_le_cong_suat || 0)}%`,
                'Trạng Thái': Number(it.ty_le_cong_suat || 0) > 100 ? 'Quá tải' : Number(it.ty_le_cong_suat || 0) >= 80 ? 'Tối ưu' : 'Còn giường'
            };
        });
        rows.push({
            'STT': 'TỔNG CỘNG',
            ...
            'Tổng Số BN Đang Nằm': totals.bn_dang_nam,
    ...
    return (
        <div className="space-y-3 sm:space-y-3.5">
            ...
            <PrintReportHeader 
                ...
                subtitle="Thống kê tỷ lệ sử dụng giường kế hoạch và cơ cấu người bệnh điều trị theo khoa"
                fromDate={fromDate}
                toDate={toDate}
            />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-3.5 print:hidden">
                ...
                <div className="his-kpi-card border-l-4 border-l-purple-500 p-4.5 rounded-xl bg-white dark:bg-slate-800 shadow-xs">
                    ...
                    <div className="mt-2 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                        <span>Nội trú: <strong className="text-purple-600 font-mono">{totals.bn_noi_tru_bhyt + totals.bn_noi_tru_vienphi}</strong> (BHYT: {totals.bn_noi_tru_bhyt})</span>
                        <span>Ngoại trú: <strong className="text-amber-600 font-mono">{totals.bn_ngoai_tru}</strong></span>
                    </div>
                </div>
                ...
                <div className="his-kpi-card border-l-4 border-l-emerald-500 p-4.5 rounded-xl bg-white dark:bg-slate-800 shadow-xs">
                    ...
                    <div className="mt-2 space-y-1">
                        ...
                        <div className="flex justify-between text-[11px] text-slate-500 dark:text-slate-400">
                            ...
                            <span className="text-emerald-600 font-semibold">Theo Giường KH</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="his-kpi-card border-l-4 border-l-amber-500 p-4.5 rounded-xl bg-white dark:bg-slate-800 shadow-xs">
                    ...
                    <div className="mt-2 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                        ...
                        <span className="text-indigo-600 font-medium">{availableDepts.length} khoa còn giường</span>
                    </div>
                </div>
            </div>
                ...
                <div className="print:hidden p-4 bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900/50 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-xs">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="text-xs text-rose-800 dark:text-rose-300">
                            ...
                            Phát hiện <strong>{overloadedDepts.map(d => `${d.dept_name} (${d.ty_le_cong_suat}% - quá ${Number(d.bn_dang_nam) - Number(d.giuong_ke_hoach)} BN)`).join(', ')}</strong>.
                        </div>
                    </div>
                    <div className="text-xs font-semibold text-rose-700 dark:text-rose-300 bg-rose-100 dark:bg-rose-900/50 px-3 py-1.5 rounded-xl self-start sm:self-auto border border-rose-300 dark:border-rose-800">
                        Khoa còn giường: {availableDepts.map(d => `${d.dept_name} (+${Number(d.giuong_ke_hoach) - Number(d.bn_dang_nam)})`).join(' • ')}
                    </div>
                </div>
            )}
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4">
                        <div>
                            ...
                            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                                Vạch đỏ đứt nét thể hiện định mức 100%. Cột đỏ: Quá tải, Cột vàng: Tối ưu (80-100%), Cột xanh: Còn giường trống
                            </p>
                        </div>
                                ...
                                <Tooltip 
                                    ...
                                    labelFormatter={(label, payload) => {
                                        if (payload && payload.length > 0) {
                                            ...
                                            const status = item.diff < 0 ? `Thiếu ${Math.abs(item.diff)} giường` : `Còn ${item.diff} giường`;
                                            return `${item.fullName} (${item.deptId}) - ${item.bn_dang_nam} BN / ${item.giuong_ke_hoach} Giường KH (${status})`;
                                        }
                                        return label;
                                    }}
                                    contentStyle={{ 
                ...
                <div className="his-kpi-card p-5 rounded-2xl bg-white dark:bg-slate-800 shadow-xs border border-slate-200/80 dark:border-slate-700 flex flex-col justify-between">
                    ...
                    <div className="space-y-1.5 pt-2 border-t border-slate-100 dark:border-slate-700/60 text-xs">
                        <div className="flex items-center justify-between">
                            ...
                            <span className="font-mono font-semibold text-slate-700 dark:text-slate-300">{availableDepts.length} khoa</span>
                        </div>
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-1.5">
                            ...
                            <span className="font-mono font-bold text-emerald-600">{optimalDepts.length} khoa</span>
                        </div>
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-1.5">
                            ...
                            <span className="font-mono font-bold text-rose-600">{overloadedDepts.length} khoa</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 p-3 bg-white dark:bg-slate-800 rounded-xl border border-slate-200/80 dark:border-slate-700 shadow-xs print:hidden">
                <div className="flex flex-wrap items-center gap-2 flex-1">
                    <div className="relative min-w-[220px] max-w-sm flex-1">
                        <input
                            ...
                            placeholder="Tìm tên khoa (gõ không dấu: ngoai, hoi suc, nhi...)"
                            className="w-full px-3.5 py-1.5 pl-9 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
                        />
                    ...
                    <div className="flex flex-wrap items-center gap-1 bg-slate-100 dark:bg-slate-900/70 p-1 rounded-lg">
                        <button
                            type="button"
                        ...
                        >
                            ...
                            <span>Quá Tải ({overloadedDepts.length})</span>
                        </button>
                        <button
                            type="button"
                        ...
                        >
                            ...
                            <span>Tối Ưu ({optimalDepts.length})</span>
                        </button>
                        <button
                            type="button"
                        ...
                        >
                            ...
                            <span>Còn Giường ({availableDepts.length})</span>
                        </button>
                    </div>

                    <label className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-400 cursor-pointer select-none ml-1">
                        <input
                            type="checkbox"
                            checked={hideEmpty}
                        ...
                        />
                        <span>Ẩn khoa không có giường/BN</span>
                    </label>
                </div>
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        onClick={fetchData}
                        disabled={loading}
                        className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 text-slate-700 dark:text-slate-200 transition cursor-pointer"
                    ...
                    <button
                        ...
                        className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold bg-emerald-50 dark:bg-emerald-950/50 hover:bg-emerald-100 text-emerald-700 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800 transition cursor-pointer"
                    ...
                    <button
                        ...
                        className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold bg-blue-50 dark:bg-blue-950/50 hover:bg-blue-100 text-blue-700 dark:text-blue-300 border border-blue-300 dark:border-blue-800 transition cursor-pointer"
                        title="In báo cáo biểu mẫu"
                    >
                        <span>🖨️</span>
            ...
            {/* Table (gcv-ninhbinh his-table standard) - Bỏ giường thực kê, tách BN nội trú BHYT/Viện phí & BN ngoại trú */}
            <div className="his-table-wrap">
                <table className="his-table w-full text-left text-xs sm:text-sm whitespace-nowrap">
                    <thead>
                        <tr>
                            <th rowSpan={2} className="px-4 py-3 text-center w-14 font-mono align-middle">STT</th>
                            <th 
                                rowSpan={2}
                                className="px-5 py-3 cursor-pointer hover:opacity-80 transition select-none align-middle"
                            ...
                            <th 
                                rowSpan={2}
                                className="px-4 py-3 text-right cursor-pointer hover:opacity-80 transition select-none align-middle"
                            ...
                            <th colSpan={2} className="px-4 py-2 text-center font-bold bg-blue-50/80 dark:bg-blue-950/40 text-blue-800 dark:text-blue-300 border-b border-blue-200 dark:border-blue-800">
                                BỆNH NHÂN NỘI TRÚ
                            </th>
                            <th 
                                rowSpan={2}
                                className="px-4 py-3 text-right cursor-pointer hover:opacity-80 transition select-none align-middle"
                                onClick={() => handleSort('bn_ngoai_tru')}
                            >
                                <div className="flex items-center justify-end gap-1.5 font-bold text-amber-600 dark:text-amber-400">
                                    <span>BN Ngoại Trú</span>
                                    {sortField === 'bn_ngoai_tru' && (<span>{sortAsc ? '▲' : '▼'}</span>)}
                                </div>
                            </th>
                            <th 
                                rowSpan={2}
                            ...
                            >
                                <div className="flex items-center justify-end gap-1.5 font-bold text-purple-600 dark:text-purple-400">
                                    <span>Tổng BN Đang Nằm</span>
                                    {sortField === 'bn_dang_nam' && (<span>{sortAsc ? '▲' : '▼'}</span>)}
                                </div>
                            </th>
                            <th 
                                rowSpan={2}
                                className="px-5 py-3 min-w-[220px] cursor-pointer hover:opacity-80 transition select-none align-middle"
                            ...
                            <th rowSpan={2} className="px-4 py-3 text-center w-32 align-middle">Trạng Thái</th>
                        </tr>
                        <tr>
                            <th 
                                className="px-3 py-2 text-right cursor-pointer hover:opacity-80 transition select-none bg-blue-50/50 dark:bg-blue-950/20 text-emerald-600 dark:text-emerald-400 font-semibold"
                                onClick={() => handleSort('bn_noi_tru_bhyt')}
                            >
                                <div className="flex items-center justify-end gap-1">
                                    <span>BHYT</span>
                                    {sortField === 'bn_noi_tru_bhyt' && (<span>{sortAsc ? '▲' : '▼'}</span>)}
                                </div>
                            </th>
                            <th 
                                className="px-3 py-2 text-right cursor-pointer hover:opacity-80 transition select-none bg-blue-50/50 dark:bg-blue-950/20 text-slate-600 dark:text-slate-300 font-semibold"
                                onClick={() => handleSort('bn_noi_tru_vienphi')}
                            >
                                <div className="flex items-center justify-end gap-1">
                                    <span>Viện Phí</span>
                                    {sortField === 'bn_noi_tru_vienphi' && (<span>{sortAsc ? '▲' : '▼'}</span>)}
                                </div>
                            </th>
                        </tr>
                    </thead>
                            ...
                            filteredItems.map((it, idx) => {
                                ...
                                return (
                                    <tr key={it.dept_id} className="hover:bg-slate-50/80 dark:hover:bg-slate-700/50 transition">
                                        ...
                                        <td className="px-3 py-3.5 text-right text-slate-600 dark:text-slate-300 font-mono tabular-nums">
                                            {Math.max(0, Number(it.bn_noi_tru_vienphi || 0)).toLocaleString('vi-VN')}
                                        </td>
                                        <td className="px-4 py-3.5 text-right text-amber-600 dark:text-amber-400 font-semibold font-mono tabular-nums">
                                            {Math.max(0, Number(it.bn_ngoai_tru || 0)).toLocaleString('vi-VN')}
                                        </td>
                    ...
                    <tfoot className="bg-slate-100 dark:bg-slate-700 font-bold text-slate-900 dark:text-white border-t-2 border-slate-300 dark:border-slate-600 sticky bottom-0 z-10 shadow-md">
                        <tr>
                            ...
                            <td className="px-4 py-3.5 text-right font-mono tabular-nums text-blue-700 dark:text-blue-300 font-bold">
                                {totals.giuong_ke_hoach.toLocaleString('vi-VN')}
                            </td>
                            <td className="px-3 py-3.5 text-right font-mono tabular-nums text-emerald-600 dark:text-emerald-300 font-bold">
                                {totals.bn_noi_tru_bhyt.toLocaleString('vi-VN')}
                            </td>
                            <td className="px-3 py-3.5 text-right font-mono tabular-nums text-slate-700 dark:text-slate-200">
                                {totals.bn_noi_tru_vienphi.toLocaleString('vi-VN')}
                            </td>
                            <td className="px-4 py-3.5 text-right font-mono tabular-nums text-amber-600 dark:text-amber-300 font-bold">
                                {totals.bn_ngoai_tru.toLocaleString('vi-VN')}
                            </td>
