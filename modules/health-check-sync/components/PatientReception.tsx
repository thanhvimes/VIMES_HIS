...
import { SearchIcon, UserGroupIcon, RefreshIcon, CheckCircleIcon, PrinterIcon, PlusIcon } from '../../../components/Icons';
...
const PatientReception: React.FC = () => {
    ...
    // State Thêm mới nhân viên tại Tiếp đón
    ...
    const [addEmployeeWards, setAddEmployeeWards] = useState<CatalogItem[]>([]);
    ...
    useEffect(() => {
        if (addEmployeeFormData.provId) {
            getWards(addEmployeeFormData.provId).then(data => {
                setAddEmployeeWards(data.map((w: any) => ({ id: String(w.id || ''), code: String(w.code || w.id || ''), name: w.name })));
            }).catch(() => setAddEmployeeWards([]));
        } else {
            setAddEmployeeWards([]);
        }
    }, [addEmployeeFormData.provId, getWards]);

    const handleOpenAddEmployeeModal = () => {
        setAddEmployeeContractId(selectedContractId || (contracts[0] ? String(contracts[0].id) : ''));
    ...
    const handleAddEmployeeSubmit = async (e: React.FormEvent) => {
        ...
        if (!addEmployeeContractId) {
            toast.error("Vui lòng chọn gói khám / hợp đồng!");
            return;
        }
        ...
        const payload = {
            ...
            address: addEmployeeFormData.address.trim(),
            note: addEmployeeFormData.note.trim(),
            guardianName: addEmployeeFormData.guardianName.trim(),
            guardianCccd: addEmployeeFormData.guardianCccd.trim()
        };

        try {
            toast.loading("Đang thêm nhân viên mới vào gói khám...");
            const res = await healthCheckService.createEmployee(payload);
            toast.dismiss();

            if (res && res.success) {
                toast.success("Thêm nhân viên vào gói khám thành công!");
                setIsAddEmployeeModalOpen(false);

                // Tự động chuyển sang xem hợp đồng vừa thêm
                setSelectedContractId(addEmployeeContractId);

                // Nạp lại danh sách nhân viên để hiển thị ngay
                const updatedEmployees = await healthCheckService.searchEmployeeByCard('', addEmployeeContractId);
                setSearchResults(updatedEmployees);

                // Tự động chọn nhân viên vừa thêm
                if (res.employeeId) {
                    const newEmp = updatedEmployees.find(emp => emp.id === res.employeeId);
                    if (newEmp) {
                        setSelectedEmployee(newEmp);
                    }
                }
            } else {
                toast.error(res?.message || "Thao tác thêm nhân viên thất bại!");
            }
        } catch (err: any) {
            toast.dismiss();
            toast.error(err.response?.data?.message || err.message || "Lỗi khi thêm nhân viên");
        }
    };
                ...
                <div className="bg-white dark:bg-slate-800 p-5 rounded-[2rem] border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col gap-4">
                    <div className="flex items-center justify-between gap-2">
                        ...
                        <button
                            ...
                            title="Thêm nhân viên mới vào gói khám"
                        >
            ...
            {isAddEmployeeModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/70 overflow-hidden animate-fade-in">
                    <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-lg w-full shadow-2xl border border-slate-100 dark:border-slate-800/80 overflow-hidden flex flex-col max-h-[90vh] animate-zoom-in">
                        ...
                        {/* Form Body */}
                        <form onSubmit={handleAddEmployeeSubmit} className="flex flex-col flex-1 overflow-hidden">
                            <div className="p-6 flex flex-col gap-4 overflow-y-auto custom-scrollbar text-xs flex-1">
