    ...
    return (
        <div className="h-full flex flex-col gap-6 animate-fade-in pb-10">
                ...
                <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
                    <div className="flex items-center bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-1">
                        <input 
                            ...
                            className="bg-transparent text-xs font-bold p-1.5 outline-none"
                        />
                    ...
                    <button 
                        onClick={loadStats}
                        disabled={isLoading}
                        className="p-2.5 bg-slate-100 dark:bg-slate-700 rounded-xl text-slate-600 hover:bg-white transition shadow-sm"
                    >
                        <RefreshIcon className={`w-5 h-5 ${isLoading ? 'animate-spin' : ''}`}/>
                    </button>
                    <button className="px-6 py-2.5 bg-teal-600 hover:bg-teal-700 text-white rounded-xl font-bold text-xs shadow-lg flex items-center gap-2 transition active:scale-95">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <KPICard title="Tỷ lệ Đến khám" value={`${stats?.kpis.arrived.toLocaleString()}`} subtext="Khách hàng thực tế" icon={TrendingUpIcon} color="bg-indigo-600" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 bg-white dark:bg-slate-800 p-6 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col min-h-[400px]">
                    <h3 className="font-black text-slate-800 dark:text-white uppercase tracking-tight text-sm mb-8 flex items-center gap-2">
                        <TrendingUpIcon className="w-5 h-5 text-blue-600"/> Biểu đồ xu hướng đăng ký và đến khám
                    </h3>
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col">
                    ...
                    <div className="mt-4 p-4 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-700">
                        ...
                        <p className="text-xs text-slate-600 dark:text-slate-400 italic">Dữ liệu cho thấy kênh Portal và Mobile App vẫn chiếm tỷ trọng lớn trong cơ cấu bệnh nhân trực tuyến.</p>
                    </div>
                </div>
            </div>
                                        ...
                                        <td className="p-4 text-center">
                                            <div className="flex items-center justify-center gap-2">
                                                <div className="w-12 bg-slate-100 dark:bg-slate-700 h-1.5 rounded-full overflow-hidden">
                                                    <div className="bg-indigo-500 h-full" style={{ width: `${Math.round((row.arrived/row.bookings)*100)}%` }}></div>
                                                </div>
                                                <span className="text-[10px] font-bold text-slate-500">{Math.round((row.arrived/row.bookings)*100)}%</span>
                                            </div>
                                        </td>
