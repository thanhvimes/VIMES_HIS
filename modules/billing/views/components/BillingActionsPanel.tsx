            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 flex-1 flex flex-col p-4">
                ...
                <div className="space-y-3">
                    <button 
                        ...
                        className="w-full py-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-bold shadow-lg flex items-center justify-center gap-2 transition-transform active:scale-95 uppercase tracking-wide"
                    >
                        <CheckCircleIcon className="w-6 h-6"/> Quyết toán / Thu tiền
                    </button>
                    
                    <button 
                        onClick={() => onAction('deposit')}
                        className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold shadow-sm flex items-center justify-center gap-2 transition-transform active:scale-95"
                    >
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-2">
                        <button 
                            ...
                            className="py-3 bg-purple-100 hover:bg-purple-200 dark:bg-purple-900/30 dark:hover:bg-purple-900/50 text-purple-700 dark:text-purple-300 rounded-lg font-semibold text-sm flex flex-col items-center justify-center gap-1 transition"
                        >
                            <TagIcon className="w-5 h-5"/> Miễn giảm
                        </button>
                        <button 
                            onClick={() => onAction('add_service')}
                            className="py-3 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 rounded-lg font-semibold text-sm flex flex-col items-center justify-center gap-1 transition"
                        >
                            <PlusIcon className="w-5 h-5"/> Thêm phí
                        </button>
                        <button 
                            onClick={() => onAction('print')}
                            className="py-3 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 rounded-lg font-semibold text-sm flex flex-col items-center justify-center gap-1 transition"
                        >
                            <PrinterIcon className="w-5 h-5"/> In Bảng kê
                        </button>
                         <button 
                            onClick={() => onAction('print_receipt')}
                            className="py-3 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 rounded-lg font-semibold text-sm flex flex-col items-center justify-center gap-1 transition"
                        >
                            <DocumentTextIcon className="w-5 h-5"/> In Biên lai
                        </button>
                    </div>
                </div>
            </div>
