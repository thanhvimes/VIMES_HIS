...
import { BeakerIcon, ListBulletIcon } from '../../../components/Icons';
...
const DashboardView: React.FC = () => {
  return (
    <div className="space-y-6">
      <p className="text-slate-500 dark:text-slate-400 -mt-2">Tổng quan tình hình kết quả xét nghiệm.</p>
      
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <DashboardCard title="Tổng số xét nghiệm" value="15" icon={<ListBulletIcon className="w-6 h-6 text-white"/>} color="bg-cyan-500" />
        <DashboardCard title="Kết quả đang chờ" value="3" icon={<BeakerIcon className="w-6 h-6 text-white"/>} color="bg-amber-500" />
        <DashboardCard title="Hoàn thành hôm nay" value="12" icon={<BeakerIcon className="w-6 h-6 text-white"/>} color="bg-emerald-500" />
      </div>
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto">
                <div className="flex justify-between items-center text-sm p-2 rounded-md hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    <div>
                        ...
                        <p className="text-slate-500 dark:text-slate-400 text-xs">Xét nghiệm nước tiểu</p>
                    </div>
