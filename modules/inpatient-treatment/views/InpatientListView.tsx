...
import { SearchIcon, UserGroupIcon, ClockIcon, CheckIcon, HospitalIcon, PlayIcon, BedIcon, ClipboardListIcon } from '../../../components/Icons';
import { useTheme } from '../../../contexts/ThemeContext';

// Mock data specific for Inpatient List
const mockInpatientList = [
  { id: 'P003', name: 'Lê Hoàng Cường', age: 45, gender: 'Nam', diagnosis: 'Viêm phổi', room: '301', bed: '02', status: 'admitted', admissionDate: '15/11/2023 08:30', priority: 'High' },
  { id: 'P004', name: 'Phạm Thị Dung', age: 22, gender: 'Nữ', diagnosis: 'Viêm ruột thừa cấp', room: '302', bed: '01', status: 'admitted', admissionDate: '16/11/2023 10:00', priority: 'Normal' },
  { id: 'P001', name: 'Nguyễn Văn An', age: 35, gender: 'Nam', diagnosis: 'Sốt xuất huyết', room: '301', bed: '01', status: 'discharged', admissionDate: '10/11/2023 14:00', priority: 'Normal' },
  { id: 'P005', name: 'Hoàng Văn Em', age: 12, gender: 'Nam', diagnosis: 'Viêm phế quản', room: '303', bed: '05', status: 'admitted', admissionDate: '17/11/2023 09:15', priority: 'Emergency' },
  { id: 'P006', name: 'Trần Thị Bích', age: 31, gender: 'Nữ', diagnosis: 'Hậu phẫu lấy thai', room: '304', bed: '02', status: 'pending_discharge', admissionDate: '12/11/2023 07:45', priority: 'Normal' },
];
...
const InpatientListView: React.FC = () => {
  ...
  const getPriorityBadge = (priority: string) => {
      ...
      if (priority === 'High') return <span className="ml-2 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded bg-orange-100 text-orange-600 border border-orange-200">Nặng</span>;
      return null;
  }
    ...
    <div className="flex flex-col h-full space-y-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm">Theo dõi bệnh nhân đang nằm viện tại khoa.</p>
        </div>
      ...
      <div className="flex-1 bg-surface dark:bg-dark-surface rounded-xl shadow-lg border border-slate-200/50 dark:border-slate-700 flex flex-col overflow-hidden">
        ...
        <div className="flex border-b border-slate-200 dark:border-slate-700">
            ...
            <button
                onClick={() => setActiveTab('pending_discharge')}
                className={`flex-1 py-4 font-medium text-center transition-colors relative ${fontSettings.controls} ${
                    activeTab === 'pending_discharge' 
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                    {filteredList.length > 0 ? (
                        filteredList.map((patient) => (
                            <tr key={patient.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer" onClick={() => handleOpenRecord(patient.id)}>
                                ...
                                <td className="p-4">
                                    <span className="bg-indigo-100 text-indigo-800 dark:bg-indigo-900/40 dark:text-indigo-300 px-2 py-1 rounded font-mono text-xs font-bold flex items-center w-fit gap-1">
                                       <BedIcon className="w-3 h-3"/> P.{patient.room} - G.{patient.bed}
                                    </span>
                                </td>
                                ...
                                <td className="p-4 text-right">
                                    <button 
                                        onClick={(e) => { e.stopPropagation(); handleOpenRecord(patient.id); }}
                                        className="inline-flex items-center px-3 py-1.5 bg-white border border-indigo-200 dark:bg-slate-800 dark:border-indigo-800 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300 text-xs font-medium rounded-md transition-colors shadow-sm"
                                    >
