
import { Patient, ExaminationRecord } from '../../types';

const nguyenVanAnHistory: ExaminationRecord[] = [
  {
    id: 'EXAM003',
    recordNumber: '21024061',
    examDate: '27/10/2023',
    clinic: 'Phòng Khám Nội',
    ticketNumber: '12',
    doctor: 'ntthien',
    status: 'Đã kết thúc',
    diagnosis: 'Rối loạn tiêu hóa',
    patientStatus: 'Không khỏe',
    examType: 'Khám Nội',
    symptoms: 'Đầy hơi, khó tiêu'
  },
];

const phamThiDungHistory: ExaminationRecord[] = [
  {
    id: 'EXAM001',
    recordNumber: '23011618',
    examDate: '09/05/2023',
    clinic: 'Phòng Khám Sản - Phụ Khoa',
    ticketNumber: '9',
    doctor: 'lvduong',
    status: 'Đã kết thúc',
    diagnosis: 'Viêm lộ tuyến cổ tử cung',
    patientStatus: 'Bình thường',
    examType: 'Khám Phụ sản',
    symptoms: 'Khí hư bất thường'
  },
  {
    id: 'EXAM002',
    recordNumber: '21024067',
    examDate: '17/11/2021',
    clinic: 'Phòng Khám Sản - Phụ Khoa',
    ticketNumber: '1',
    doctor: 'ntthien',
    status: 'Đã kết thúc',
    diagnosis: 'Theo dõi thai có nguy cơ',
    patientStatus: 'Không khỏe',
    examType: 'Khám thai',
    symptoms: 'Ra máu nhẹ'
  }
];

export const mockPatients: Patient[] = [
  {
    id: 'P001',
    recordNumber: '21024061',
    name: 'Nguyễn Văn An',
    dob: '20/05/1988',
    age: 35,
    gender: 'Nam',
    ethnicity: 'Kinh',
    occupation: 'Văn phòng',
    address: '123 Đường Giải Phóng, Hai Bà Trưng, Hà Nội',
    phone: '0912345678',
    identityCard: '012345678901',
    relativeInfo: 'Trần Thị Bích (Vợ)',
    lastVisit: '27/10/2023',
    patientType: 'Dịch vụ',
    history: nguyenVanAnHistory,
    examinationStatus: 'waiting',
    assignedDoctor: 'BS. Trần Thị B'
  },
  {
    id: 'P002',
    recordNumber: '21024062',
    name: 'Trần Thị Bích',
    dob: '15/11/1992',
    age: 31,
    gender: 'Nữ',
    ethnicity: 'Kinh',
    occupation: 'Giáo viên',
    address: '123 Đường Giải Phóng, Hai Bà Trưng, Hà Nội',
    phone: '0987654321',
    lastVisit: '27/10/2023',
    patientType: 'Bảo hiểm',
    examinationStatus: 'processing',
    assignedDoctor: 'BS. Nguyễn Văn A'
  },
  {
    id: 'P003',
    recordNumber: '21024067',
    name: 'Lê Hoàng Cường',
    dob: '10/02/1978',
    age: 45,
    gender: 'Nam',
    ethnicity: 'Kinh',
    occupation: 'Kỹ sư',
    address: '456 Đường Minh Khai, Hoàng Mai, Hà Nội',
    phone: '0905123456',
    lastVisit: '26/10/2023',
    patientType: 'Dịch vụ',
    examinationStatus: 'completed',
    assignedDoctor: 'BS. Lê Văn C'
  },
  {
    id: 'P004',
    recordNumber: '23011618',
    name: 'Phạm Thị Dung',
    dob: '30/07/2001',
    age: 22,
    gender: 'Nữ',
    ethnicity: 'Kinh',
    occupation: 'Sinh viên',
    address: '789 Đường Trường Chinh, Đống Đa, Hà Nội',
    phone: '0358987654',
    lastVisit: '25/10/2023',
    patientType: 'Dịch vụ',
    history: phamThiDungHistory,
    examinationStatus: 'waiting',
    assignedDoctor: 'BS. Phạm Văn D'
  },
];
