    ...
    return (
        <div className="h-screen overflow-hidden bg-slate-50 font-sans flex flex-col relative">
            ...
            <header className="hidden md:flex bg-teal-700 text-white px-8 py-4 justify-between items-center shadow-md sticky top-0 z-30">
                ...
                <div className="flex items-center gap-4">
                    <div className="text-right hidden lg:block">
                        <p className="text-sm font-bold">{patient?.name?.toUpperCase() || 'BỆNH NHÂN'}</p>
                        <p className="text-xs text-teal-200">BN: {patient?.id || 'N/A'}</p>
                    </div>
            ...
            <header className={`md:hidden px-4 py-3 flex items-center justify-between ${isHome ? 'bg-teal-600 text-white' : 'bg-white text-slate-800 border-b border-slate-200'} transition-colors sticky top-0 z-20 shadow-sm`}>
                {isHome ? (
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="font-bold text-sm">{patient?.name?.toUpperCase() || 'BỆNH NHÂN'}</p>
                        </div>
                    </div>
                ) : (
                ...
                <button onClick={logout} className={`p-2 rounded-full ${isHome ? 'hover:bg-white/20 text-white' : 'hover:bg-slate-100 text-slate-500'}`}>
            ...
            <main className="flex-1 relative overflow-hidden w-full max-w-md md:max-w-7xl mx-auto md:p-6">
                <Outlet />
            </main>
