...
const LabResultTemplate: React.FC<LabResultTemplateProps> = ({ data }) => {
    ...
    if (!labData) return <div className="text-center text-slate-400 p-10 text-lg">Chưa có dữ liệu kết quả chi tiết.</div>;

    const handlePrint = () => {
        openPdf({
            url: DEMO_PDF_URL,
            fileName: `LabResult_${data.id}.pdf`,
            isSignable: true
        });
    };
    ...
    return (
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden flex flex-col h-full">
            ...
            <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50 flex-shrink-0">
                <div>
                    <h3 className="text-xl font-bold text-blue-700 dark:text-blue-400 uppercase flex items-center gap-2">
                        <BeakerIcon className="w-6 h-6"/> {data.name}
                    </h3>
            ...
            <div className="px-6 py-4 bg-white dark:bg-slate-800 border-b border-slate-100 dark:border-slate-700 flex-shrink-0">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {/* Column 1: Chỉ định */}
                    <div className="p-4 bg-slate-50 dark:bg-slate-900/30 rounded-lg border border-slate-100 dark:border-slate-700/50">
                        <h4 className="text-sm font-bold text-blue-600 dark:text-blue-400 mb-3 uppercase border-b border-slate-200 dark:border-slate-700 pb-1">Thông tin chỉ định</h4>
                    ...
                    {/* Column 2: Kết quả */}
                    <div className="p-4 bg-slate-50 dark:bg-slate-900/30 rounded-lg border border-slate-100 dark:border-slate-700/50">
                        <h4 className="text-sm font-bold text-blue-600 dark:text-blue-400 mb-3 uppercase border-b border-slate-200 dark:border-slate-700 pb-1">Thông tin kết quả</h4>
                    ...
                    {/* Column 3: Bệnh phẩm */}
                    <div className="p-4 bg-slate-50 dark:bg-slate-900/30 rounded-lg border border-slate-100 dark:border-slate-700/50">
                        <h4 className="text-sm font-bold text-blue-600 dark:text-blue-400 mb-3 uppercase border-b border-slate-200 dark:border-slate-700 pb-1">Thông tin bệnh phẩm</h4>
            ...
            <div className="flex-1 overflow-auto p-0">
                <table className="w-full text-base text-left">
                    <thead className="bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-200 font-bold uppercase text-sm sticky top-0 z-10 shadow-sm">
                        <tr>
                            <th className="px-6 py-4">Tên chỉ số</th>
                            <th className="px-6 py-4 text-right">Kết quả</th>
                            <th className="px-6 py-4">Đơn vị</th>
                            <th className="px-6 py-4">Trị số bình thường</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                        {labData.items.map((item, idx) => (
                            <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                ...
                                <td className={`px-6 py-4 text-right font-bold ${item.isAbnormal ? 'text-red-600 dark:text-red-400' : 'text-blue-600 dark:text-blue-400'}`}>
                                    {item.result}
                                </td>
