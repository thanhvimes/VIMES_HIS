      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-surface dark:bg-dark-surface p-6 rounded-xl shadow-lg border border-slate-200/50 dark:border-slate-700">
             ...
             <div className="space-y-4">
                <div>
                    <label className={`font-semibold text-slate-600 dark:text-slate-300 block mb-1 ${fontSettings.controls}`}>Triệu chứng</label>
                    <textarea value={currentSymptoms} onChange={e => setCurrentSymptoms(e.target.value)} rows={3} className={`w-full p-2 bg-inherit border border-slate-300 dark:border-slate-600 rounded-md focus:ring-2 focus:ring-primary focus:border-primary ${fontSettings.controls}`}></textarea>
                </div>
                <div>
                    <label className={`font-semibold text-slate-600 dark:text-slate-300 block mb-1 ${fontSettings.controls}`}>Ghi chú/Kết quả khám</label>
                    <textarea value={currentNotes} onChange={e => setCurrentNotes(e.target.value)} rows={4} className={`w-full p-2 bg-inherit border border-slate-300 dark:border-slate-600 rounded-md focus:ring-2 focus:ring-primary focus:border-primary ${fontSettings.controls}`}></textarea>
                </div>
                 <div>
                    <label className={`font-semibold text-slate-600 dark:text-slate-300 block mb-1 ${fontSettings.controls}`}>Chẩn đoán</label>
                    <input type="text" value={currentDiagnosis} onChange={e => setCurrentDiagnosis(e.target.value)} className={`w-full p-2 bg-inherit border border-slate-300 dark:border-slate-600 rounded-md focus:ring-2 focus:ring-primary focus:border-primary ${fontSettings.controls}`} />
                </div>
        ...
        <div className="space-y-6">
           <div className="bg-gradient-to-br from-cyan-50 to-blue-100 dark:from-cyan-900/50 dark:to-blue-900/50 p-6 rounded-xl shadow-lg border border-slate-200/50 dark:border-slate-700">
                ...
                <button onClick={handleGetAISuggestions} disabled={isLoadingAI} className={`w-full bg-primary hover:bg-primary-dark text-white font-bold py-2 px-4 rounded-lg shadow-md transition-transform transform hover:scale-105 disabled:bg-slate-400 disabled:cursor-not-allowed flex items-center justify-center ${fontSettings.controls}`}>
                    {isLoadingAI ? (
                ...
                {aiSuggestion && (
                    <div className={`mt-4 space-y-3 text-slate-700 dark:text-slate-300 ${fontSettings.listSecondary}`}>
                       <div>
           ...
           <div className="bg-surface dark:bg-dark-surface p-6 rounded-xl shadow-lg border border-slate-200/50 dark:border-slate-700">
                ...
                <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2">
                    {history.map(rec => (
                        <div key={rec.id} className={`border-b border-slate-100 dark:border-slate-700 pb-2 ${fontSettings.listSecondary}`}>
