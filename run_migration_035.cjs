...
async function executeMigration() {
  ...
  try {
    ...
    const migrationFile = path.join(__dirname, 'backend/migrations/035_upgrade_db_qd2062.sql');
    ...
    const columnsCheck = await client.query(`
      ...
      WHERE table_name = 'health_check_masters' AND column_name IN ('guardian_name', 'guardian_cccd')
    `);
    
    console.log('\n--- VERIFICATION ---');
    console.log('Columns added to health_check_masters:', columnsCheck.rows);
