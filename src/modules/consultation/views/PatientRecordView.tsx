    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900 overflow-hidden relative">
            ...
            <div className="flex-shrink-0 bg-gradient-to-r from-teal-700 to-teal-600 dark:from-slate-900 dark:to-slate-800 text-white shadow-md z-20 border-b dark:border-slate-700">
                <div className="flex items-center justify-between px-4 py-2">
                    <div className="flex items-center space-x-4">
                        ...
                        <div className="flex flex-col">
                            <h1 className="text-lg font-bold uppercase flex items-center gap-2 flex-wrap">
                                <span className="w-2 h-2 bg-green-400 rounded-full animate-pulse flex-shrink-0"></span>
                                {patientData.name} <span className="mx-1 opacity-50 hidden sm:inline">|</span> 
                                <span className="text-base font-normal sm:font-bold">Tuổi: {patientData.age}</span>
                                <span className="mx-1 opacity-50 hidden sm:inline">|</span>
                                <span className="text-base font-normal sm:font-bold">{patientData.gender}</span>
                            </h1>
                            <p className="text-xs text-teal-100 dark:text-slate-400 opacity-90 flex items-center gap-1">
                                <span className="opacity-70 flex-shrink-0">📍 Địa chỉ:</span> <span className="truncate max-w-[200px] sm:max-w-md">{patientData.address}</span>
                            </p>
                        </div>
                    </div>
                    <div className="hidden md:flex items-center gap-3">
                        ...
                        <div className="text-sm font-bold bg-white/20 px-3 py-1 rounded-full backdrop-blur-sm max-w-md truncate border border-white/10" title={patientData.diagnosis}>
                ...
                {/* 2. NAVIGATION TABS */}
                <div className="flex items-end px-2 pt-1 space-x-1 overflow-x-auto no-scrollbar">
                    {tabs.map(tab => (
                        <button
                            key={tab.id}
                            onClick={() => setActiveTab(tab.id)}
                            className={`flex flex-col items-center justify-center py-2 px-6 min-w-[90px] rounded-t-lg transition-all duration-200 border-b-4 flex-shrink-0 ${activeTab === tab.id
