-- Migration 072: Sửa lỗi khóa ngoại hms_doc_hms_patient_fk1 và đồng bộ bệnh nhân KSK vào hms_patient
-- Description: Dọn dẹp mã bệnh nhân rác/mồ côi trong hms_exm_employee và nâng cấp stored procedure hms_exm_registration_exam tự kiểm tra và tạo hms_patient nếu chưa tồn tại

-- 1. Dọn dẹp các mã bệnh nhân rác/mồ côi trong bảng hms_exm_employee (chưa tiếp đón và không tồn tại trong hms_patient)
UPDATE hms_exm_employee
SET hee_patientno = NULL
WHERE hee_patientno IS NOT NULL 
  AND hee_patientno > 0
  AND (hee_docno IS NULL OR hee_docno = 0)
  AND NOT EXISTS (
      SELECT 1 FROM hms_patient WHERE hp_patientno = hms_exm_employee.hee_patientno
  );

-- 2. Cập nhật stored procedure hms_exm_registration_exam với cơ chế fail-safe kiểm tra và tự tạo hms_patient nếu thiếu
...
BEGIN
    ...
    -- Kiểm tra tính hợp lệ của v_patientno trong hms_patient (phòng tránh vi phạm Foreign Key hms_doc_hms_patient_fk1)
    IF v_patientno IS NULL OR v_patientno <= 0 OR NOT EXISTS (SELECT 1 FROM hms_patient WHERE hp_patientno = v_patientno) THEN
        -- Tìm kiếm theo CCCD / CMND nếu có
        IF v_id IS NOT NULL AND TRIM(v_id) <> '' THEN
            SELECT hp_patientno INTO v_patientno
            FROM hms_patient
            WHERE TRIM(hp_sin) = TRIM(v_id) OR TRIM(hp_patientid) = TRIM(v_id)
            LIMIT 1;
        END IF;

        -- Nếu vẫn chưa có thì sinh mã mới và chèn hms_patient
        IF v_patientno IS NULL OR v_patientno <= 0 OR NOT EXISTS (SELECT 1 FROM hms_patient WHERE hp_patientno = v_patientno) THEN
            ...
            INSERT INTO hms_patient (
                ...
                hp_provid, hp_createdby, hp_createddate
            ) VALUES (
                v_patientno,
                COALESCE(v_id, ''),
                COALESCE(v_id, ''),
        ...
        -- Cập nhật lại mã bệnh nhân hợp lệ vào hms_exm_employee
        UPDATE hms_exm_employee
        ...
        FOR tmpRec IN (
            ...
            IF (bIsAgeOk = TRUE AND bIsDeptOk = TRUE) THEN
                ...
                SELECT p_userid,
                       ...
                       hfl_groupid,
                       hfl_feeid,
                       tmpRec.hesp_quantity,
                       tmpRec.hesp_unitprice,
                       tmpRec.hesp_quantity * tmpRec.hesp_unitprice,
                       0,
                       0,
                       0,
                       'O',
                       ...
                       'CL'
                FROM hms_fee_list
                WHERE hfl_feeid = tmpRec.hesp_itemid;
