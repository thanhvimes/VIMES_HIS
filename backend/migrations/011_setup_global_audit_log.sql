...
-- =============================================================================
-- HỆ THỐNG AUDIT LOG TOÀN CỤC (GLOBAL AUDIT LOG)
-- =============================================================================

-- 1. Bảng lưu trữ log tập trung
...
CREATE INDEX IF NOT EXISTS idx_audit_log_table_record ON sys_audit_log(table_name, record_id);
CREATE INDEX IF NOT EXISTS idx_audit_log_created_at ON sys_audit_log(created_at);

-- 2. Hàm Trigger tổng quát
CREATE OR REPLACE FUNCTION fn_sys_audit_trigger()
RETURNS TRIGGER AS $$
DECLARE
