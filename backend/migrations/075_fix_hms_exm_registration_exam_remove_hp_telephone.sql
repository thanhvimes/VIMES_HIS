-- Migration 075: Sửa stored procedure hms_exm_registration_exam - Bỏ trường hp_telephone không tồn tại trong hms_patient
-- Description: Bảng hms_patient trên HIS không có cột hp_telephone (SĐT được lưu tại hms_doc.hd_telephone và hms_exm_employee.hee_phone)
...
BEGIN
    ...
    IF v_patientno IS NULL OR v_patientno <= 0 OR NOT EXISTS (SELECT 1 FROM hms_patient WHERE hp_patientno = v_patientno) THEN
        ...
        -- Nếu vẫn chưa có thì sinh mã mới và chèn hms_patient (bỏ cột hp_telephone vì không tồn tại)
        IF v_patientno IS NULL OR v_patientno <= 0 OR NOT EXISTS (SELECT 1 FROM hms_patient WHERE hp_patientno = v_patientno) THEN
