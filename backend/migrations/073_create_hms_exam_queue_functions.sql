-- Migration: 073_create_hms_exam_queue_functions.sql
-- Description: Tạo các hàm xử lý hàng đợi khám bệnh và thông tin bệnh nhân (hms_get_exam_queue_v1, hms_get_rooms_v1, hms_get_patient_profile_v1)
...
BEGIN
    ...
    v_from_d := COALESCE(NULLIF(TRIM(p_from_date), ''), CURRENT_DATE::text);
    v_to_d := COALESCE(NULLIF(TRIM(p_to_date), ''), CURRENT_DATE::text);
    ...
    FROM (
        ...
        WHERE (p_dept_id = 'ALL' OR e.he_deptid = p_dept_id)
          ...
          AND e.he_examdate >= (v_from_d::date + v_from_time::time)
          AND e.he_examdate <= (v_to_d::date + v_to_time::time)
          AND (
...
DROP FUNCTION IF EXISTS hms_get_rooms_v1(text);
