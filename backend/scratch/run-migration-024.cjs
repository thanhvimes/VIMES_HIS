...
async function checkDb() {
  ...
  try {
    ...
    console.log('Running migration: 024_add_hec_status_to_hms_exm_contract.sql...');
    await client.query("ALTER TABLE hms_exm_contract ADD COLUMN IF NOT EXISTS hec_status VARCHAR(1) DEFAULT 'O'");
    await client.query("COMMENT ON COLUMN hms_exm_contract.hec_status IS 'Trạng thái đồng bộ HIS. O: Chưa đồng bộ, P: Đang/Đã đồng bộ'");
    console.log('✅ Column hec_status added successfully.');
  } catch (err) {
