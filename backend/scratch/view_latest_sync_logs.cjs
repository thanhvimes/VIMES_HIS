...
async function viewLogs() {
  ...
  try {
    ...
    // Check settings loaded in db
    const settings = await client.query('SELECT id, vneid_url, vneid_username, ma_cskcb FROM health_check_settings');
    console.log('--- DB SETTINGS ---');
    console.log(settings.rows);

    // Get last 3 logs
    const res = await client.query(`
      SELECT id, patient_name, doc_no, send_status, error_message, response_log, updated_at 
