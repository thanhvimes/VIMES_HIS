...
export function validateNewHealthCheckDocument(input: NewHealthCheckDocumentInput): string[] {
    ...
    const isSigning = !!input.isSigning;
    const fundingSource = String(input.fundingSource ?? '').trim();
    if (isSigning) {
        if (!fundingSource) errors.push('Nguồn chi trả là bắt buộc khi khóa/ký số');
        else if (!['1', '2', '3', '4', '5', '9'].includes(fundingSource)) errors.push('Nguồn chi trả không hợp lệ; chỉ chấp nhận mã 1, 2, 3, 4, 5 hoặc 9');
    } else if (fundingSource && !['1', '2', '3', '4', '5', '9'].includes(fundingSource)) {
        errors.push('Nguồn chi trả không hợp lệ; chỉ chấp nhận mã 1, 2, 3, 4, 5 hoặc 9');
    }
    ...
    if (isSigning) {
        if (ageGroup !== 'UNDER_6' && !fitnessClass) errors.push('Phân loại sức khỏe là bắt buộc với nhóm từ đủ 6 tuổi khi khóa/ký số');
    ...
    } else if (fitnessClass && !['1', '2', '3', '4', '5'].includes(fitnessClass)) {
