...
class CatalogController {
    ...
    async getProvinces(req: Request, res: Response) {
        try {
            const result = await query(
                `SELECT sp_id::text as id, sp_name as name FROM sys_prov WHERE sp_isactive = 'Y' ORDER BY sp_name`
            );
    ...
    async getDistricts(req: Request, res: Response) {
        try {
            ...
            const result = await query(
                `SELECT sd_id::text as id, sd_name as name FROM sys_dist WHERE sd_active = 'Y' AND sd_provid = $1 ORDER BY sd_name`,
                [provinceId]
            );
            return res.json(result.rows);
        } catch (error: any) {
    ...
    async getWards(req: Request, res: Response) {
        try {
            ...
            const result = await query(
                "SELECT sv_id::text as id, sv_name as name FROM sys_vill WHERE sv_isactive = 'Y' AND (sv_provid = $1 OR sv_distid = $1) ORDER BY sv_name",
                [provinceId]
            );
            return res.json(result.rows);
        } catch (error: any) {
