...
async function executeMigration() {
  ...
  try {
    ...
    const migrationFile = path.join(__dirname, 'backend/migrations/027_add_allow_unsigned_sync_to_health_check_settings.sql');
    ...
    const resColumns = await client.query(`
      ...
      WHERE table_name = 'health_check_settings' 
        AND column_name = 'allow_unsigned_sync'
    `);
    
    console.log('\n--- VERIFICATION ---');
    if (resColumns.rows.length > 0) {
      console.log(`Column ${resColumns.rows[0].column_name} exists with type ${resColumns.rows[0].data_type}`);
    } else {
      console.error('❌ Column allow_unsigned_sync not found after migration!');
    }
