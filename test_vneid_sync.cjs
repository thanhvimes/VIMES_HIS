...
// Database connection info resolved dynamically from backend .env
const dbConfig = {
  user: resolveSecret(process.env.DB_USER),
  password: resolveSecret(process.env.DB_PASSWORD),
  host: process.env.DB_HOST || '127.0.0.1',
  database: process.env.DB_NAME || 'vimes_bvk',
...
async function runTests() {
  ...
  try {
    ...
    try {
      ...
      console.log(`   -> Bỏ qua tạo mock (Không tìm thấy route create-mock hoặc dev environment chưa sẵn sàng)\n`);
    }
