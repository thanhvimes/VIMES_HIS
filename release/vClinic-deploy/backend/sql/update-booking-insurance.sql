-- ==================== UPDATE BOOKING FOR INSURANCE & PRIORITY ====================
-- 1. Add columns to qms_patient table
ALTER TABLE qms_patient ADD COLUMN IF NOT EXISTS qms_is_priority BOOLEAN DEFAULT FALSE;
ALTER TABLE qms_patient ADD COLUMN IF NOT EXISTS qms_is_insurance BOOLEAN DEFAULT FALSE;

-- 2. Update created_at column if missing (just in case)
-- ALTER TABLE qms_patient RENAME COLUMN qms_createddate TO created_at; -- No, keep existing schema

-- 3. Update the stored procedure to accept 22 parameters
  ...
  BEGIN
    ...
    INSERT INTO qms_patient
      (
        ...
        qms_idcard_issue_date, qms_is_priority, qms_is_insurance
      )
      VALUES
      (
        v_idx, p_cccd, p_ho_ten, p_ngay_sinh, p_gioi_tinh, p_dan_toc,
        ...
        p_idcard_issue_date, p_is_priority, p_is_insurance
      );
