...
async function executeMigration() {
  ...
  try {
    ...
    const migrationFile = path.join(__dirname, 'backend/migrations/029_add_response_log_to_health_check_masters.sql');
    ...
    if (resColumns.rows.length > 0) {
      ...
      console.error('❌ Column response_log not found after migration!');
    }
