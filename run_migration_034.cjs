...
async function executeMigration() {
  ...
  try {
    ...
    const migrationFile = path.join(__dirname, 'backend/migrations/034_add_reception_slip_and_qz_tray_to_settings.sql');
    ...
    const columnsCheck = await client.query(`
      ...
      WHERE table_name = 'health_check_settings' AND column_name IN ('reception_slip_template', 'use_qz_tray')
    `);
    
    console.log('\n--- VERIFICATION ---');
    console.log('Columns added:', columnsCheck.rows);
