...
const hexKey = fs.readFileSync('scratch/real_private_key.pem', 'utf8').trim();
...
try {
    ...
    console.log('✅ Parsed PKCS8 DER Private Key successfully');
} catch (e1) {
    try {
        const derBuffer = Buffer.from(hexKey, 'hex');
        privateKeyObj = crypto.createPrivateKey({
        ...
        console.log('✅ Parsed PKCS1 DER Private Key successfully');
    } catch (e2) {
        console.error('❌ Failed to parse DER private key:', e1.message, e2.message);
    }
}
