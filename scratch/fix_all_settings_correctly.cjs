const { Client } = require('pg');
const crypto = require('crypto');
const fs = require('fs');

const masterKey = 'vClinic-secure-master-key-2026-vimes-jsc-admin';

function encrypt(text) {
    const salt = crypto.randomBytes(64);
    const iv = crypto.randomBytes(16);
    const key = crypto.pbkdf2Sync(masterKey, salt, 100000, 32, 'sha512');
    const cipher = crypto.createCipheriv('aes-256-gcm', key, iv);
    const encrypted = Buffer.concat([cipher.update(text, 'utf8'), cipher.final()]);
    const tag = cipher.getAuthTag();
    return 'enc:' + Buffer.concat([salt, iv, tag, encrypted]).toString('hex');
}

function decrypt(cipherText) {
    if (!cipherText || !cipherText.startsWith('enc:')) return cipherText;
    const data = Buffer.from(cipherText.replace('enc:', ''), 'hex');
    const salt = data.subarray(0, 64);
    const iv = data.subarray(64, 80);
    const tag = data.subarray(80, 96);
    const text = data.subarray(96);
    const key = crypto.pbkdf2Sync(masterKey, salt, 100000, 32, 'sha512');
    const decipher = crypto.createDecipheriv('aes-256-gcm', key, iv);
    decipher.setAuthTag(tag);
    return decipher.update(text, undefined, 'utf8') + decipher.final('utf8');
}

const envText = fs.readFileSync('backend/.env', 'utf8');
const env = {};
envText.split('\n').forEach(line => {
    const parts = line.split('=');
    if (parts.length >= 2) env[parts[0].trim()] = parts.slice(1).join('=').trim();
});

const user = decrypt(env.DB_USER);
const password = decrypt(env.DB_PASSWORD);
const host = env.DB_HOST || '14.177.232.29';
const port = parseInt(env.DB_PORT || '8050', 10);
const database = env.DB_NAME || 'vimes_nb';

// Account credentials from tai_khoan.txt
const username = '8934285008135_api';
const rawPassword = 'Abc@1234';
const publicKey = ``;
const rawPrivateKeyB64 = ``;

const pemPrivateKey = `-----BEGIN PRIVATE KEY-----\n${rawPrivateKeyB64.match(/.{1,64}/g).join('\n')}\n-----END PRIVATE KEY-----`;

async function main() {
    const client = new Client({ user, password, host, port, database });
    await client.connect();

    const encPass = encrypt(rawPassword);
    const encPrivKey = encrypt(pemPrivateKey);

    await client.query(`
        UPDATE health_check_settings
        SET vneid_url = 'https://api-sandbox.emrhub.vn',
            vneid_username = $1,
            vneid_password = $2,
            ma_cskcb = '8934285008135',
            ma_gtin_cskcb = '8934285008135',
            vneid_public_key = $3,
            vneid_private_key = $4
        WHERE id = 1 OR id IS NOT NULL
    `, [username, encPass, publicKey, encPrivKey]);

    console.log('🎉 SUCCESSFULLY CONFIGURED ALL KEYS AND SETTINGS IN DB!');

    const checkRes = await client.query('SELECT * FROM health_check_settings LIMIT 1');
    const r = checkRes.rows[0];
    console.log('Verified Settings:');
    console.log('  URL:', r.vneid_url);
    console.log('  Username:', r.vneid_username);
    console.log('  Public Key (starts with):', r.vneid_public_key.substring(0, 30));
    console.log('  Private Key (decrypted starts with):', decrypt(r.vneid_private_key).substring(0, 30));

    await client.end();
}

main().catch(err => console.error(err.message));
