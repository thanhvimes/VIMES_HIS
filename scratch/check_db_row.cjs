...
async function main() {
    ...
    const res = await client.query('SELECT * FROM health_check_settings LIMIT 1');
    if (res.rows.length > 0) {
        const row = res.rows[0];
        console.log('--- DATABASE SETTINGS ROW ---');
        console.log('ma_cskcb:', row.ma_cskcb);
        console.log('vneid_url:', row.vneid_url);
        console.log('vneid_username:', row.vneid_username);
        console.log('vneid_password (decrypted):', decrypt(row.vneid_password));
