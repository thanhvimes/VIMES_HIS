...
async function test() {
  ...
  try {
    ...
    const provs = await client.query('SELECT count(*) FROM sys_prov');
    console.log('sys_prov count:', provs.rows[0].count);

    const eths = await client.query("SELECT count(*) FROM sys_sel WHERE trim(ss_id)='sys_ethnic'");
    console.log('sys_ethnic count:', eths.rows[0].count);

    const occs = await client.query("SELECT count(*) FROM sys_sel WHERE trim(ss_id)='sys_occupation'");
    console.log('sys_occupation count:', occs.rows[0].count);

    const nats = await client.query('SELECT count(*) FROM hms_quoctich');
    console.log('hms_quoctich count:', nats.rows[0].count);
