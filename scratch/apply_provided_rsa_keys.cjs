const { Client } = require('pg');
require('dotenv').config({ path: 'backend/.env' });
const SecurityUtils = require('./backend/src/utils/security').default || require('./backend/src/utils/security');

const privateKey = `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`;

const publicKey = `-----BEGIN PUBLIC KEY-----

-----END PUBLIC KEY-----`;

async function main() {
    const encPrivateKey = 'enc:' + SecurityUtils.encrypt(privateKey.trim());
    const client = new Client({
        host: process.env.DB_HOST || '14.177.232.29',
        port: process.env.DB_PORT || 8050,
        database: process.env.DB_NAME || 'vimes_nb',
        user: SecurityUtils.resolveSecret(process.env.DB_USER),
        password: SecurityUtils.resolveSecret(process.env.DB_PASSWORD)
    });

    await client.connect();
    console.log('✅ Connected to PostgreSQL database');

    await client.query(`
        UPDATE health_check_settings
        SET vneid_private_key = $1,
            vneid_public_key = $2,
            updated_at = NOW()
        WHERE id = (SELECT id FROM health_check_settings ORDER BY id ASC LIMIT 1)
    `, [encPrivateKey, publicKey.trim()]);

    console.log('🎉 Successfully updated health_check_settings with the provided RSA Private & Public Keys!');

    const res = await client.query(`SELECT id, vneid_url, vneid_username, ma_cskcb, vneid_private_key, vneid_public_key FROM health_check_settings ORDER BY id ASC LIMIT 1`);
    const row = res.rows[0];
    console.log('📊 Verified DB row:');
    console.log('   id:', row.id);
    console.log('   ma_cskcb:', row.ma_cskcb);
    console.log('   vneid_url:', row.vneid_url);
    console.log('   vneid_private_key (encrypted len):', row.vneid_private_key.length);
    console.log('   vneid_public_key (starts with):', row.vneid_public_key.substring(0, 30));

    await client.end();
}

main().catch(err => {
    console.error('❌ Error updating DB:', err);
    process.exit(1);
});
