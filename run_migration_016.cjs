...
async function executeMigration() {
  ...
  try {
    ...
    const migrationFile = path.join(__dirname, 'backend/migrations/016_health_check_settings.sql');
    ...
    // Verify settings row
    const res = await client.query('SELECT * FROM health_check_settings');
